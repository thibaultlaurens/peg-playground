...
func TestSimpleEqual(t *testing.T) {
	res, err := Parse("", []byte("something=another"))
	assert.Nil(t, err)
...
func TestObjectPath(t *testing.T) {
	res, err := Parse("", []byte("item.spec.ssd=yes"))
	assert.Nil(t, err)
...
func TestValue(t *testing.T) {
	res, err := Parse("", []byte("item.spec.ssd=512gb"))
	assert.Nil(t, err)
	q := res.(*Query)
	f := q.AQ[0].FQ[0].Field
	...
	assert.Equal(t, f.Value.(*Measure).Number, int64(512), "Value should be 512")
	assert.Equal(t, f.Value.(*Measure).Unit, "gb", "The unit should match")

}

func TestValueInt(t *testing.T) {
	res, err := Parse("", []byte("item.spec.ssd=512"))
	assert.Nil(t, err)
	q := res.(*Query)
	f := q.AQ[0].FQ[0].Field
...
func TestValueFloat(t *testing.T) {
	res, err := Parse("", []byte("item.spec.ssd = 512.1"))
	assert.Nil(t, err)
	q := res.(*Query)
	f := q.AQ[0].FQ[0].Field
...
func TestMultipleFields(t *testing.T) {
	res, err := Parse("", []byte("item.spec.ssd > 512 item.name=laptop"))
	assert.Nil(t, err)
...
func TestBracesExpression(t *testing.T) {
	res, err := Parse("", []byte("item.spec.ssd > 512 ( item.maker=asus | item.maker=coconics ) item.name=laptop "))
	assert.Nil(t, err)
